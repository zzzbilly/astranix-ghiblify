  ...
  try {
    ...
    if ("image" === a)
        ...
        throw new Error("Unknown command type for image generation.");
      {
        ...
        (r =
          ...
          (s = "Image transformed into Studio Ghibli style."),
    ...
    if (!u)
      return (
        ...
        void (await e.editReply(
          g
            ? `❌ AI merespons dengan teks: "${g}". Tidak ada gambar dihasilkan.`
            : "❌ Gagal menghasilkan gambar. Gemini tidak mengembalikan data gambar yang diharapkan."
        ))
      );
    const m = `generated-${Date.now()}.png`;
    (i = `${l}/${m}`),
    ...
    const p = new Date(),
      ...
      k = p.toLocaleTimeString("en-US", h),
      ...
      y = new EmbedBuilder()
        .setTitle(d)
        .setDescription(`**Prompt:** \`${s}\`\n`)
        .setImage(`attachment://${m}`)
        .setFooter({ text: w });
    "ghibli" === a &&
      n &&
      y.setDescription(
    ...
    if (e.replied || e.deferred)
      ...
      try {
        await e.deferReply({ flags: MessageFlags.Ephemeral }),
          await e.editReply(
            `${t} Silakan buka tiket di [Astranix #adaastranix](https://discord.gg/zVPD4Ng2fM)`
          );
      } catch (e) {
  ...
  app.get("/oauth-callback", async (e, a) => {
    ...
    try {
      ...
      const n = await axios.post(
        ...
        { access_token: i, refresh_token: r, expires_in: s } = n.data,
        o = new Date(Date.now() + 1e3 * s),
